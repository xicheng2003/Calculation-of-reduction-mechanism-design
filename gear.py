...
class gear():
    ...
    def design(self,P,n,i):
        ...
        if gear.number==1:
            z1=23
            self.material={'小齿轮':{'牌号':'40Cr','热处理方法':'调质','齿面硬度':'280HBW','接触疲劳极限':600,'弯曲疲劳极限':500},
                     '大齿轮':{'牌号':'45钢','热处理方法':'调质','齿面硬度':'240HBW','接触疲劳极限':550,'弯曲疲劳极限':320}
                   }
        ...
        print('\t   选择大齿轮材料为{}({})，齿面硬度{}'.format(self.material['大齿轮']['牌号'],self.material['大齿轮']['热处理方法'],self.material['大齿轮']['齿面硬度']))
        print('\t(4)初选小齿轮齿数 ')
...
####按齿面接触疲劳强度设计####
        ...
        j=1
        Lh=2*8*300*8#两班制 每年按300天计算 工作寿命八年
        N1=60*n1*j*Lh
        N2=N1/u
        if gear.number==1:
            K_HN1=(10**9/N1)**0.0706#查图10-19可得接触疲劳寿命系数
            K_HN2=(10**9/N2)**0.0706#查图10-19可得
        else :
            K_HN1=(10**9/N1)**0.057#查图10-19可得
            K_HN2=(10**9/N2)**0.057#查图10-19可得
        ...
        if K_A*F_t1/b>=100:
            ...
            K_H_alpha=1.4#
        if gear.number==1:
            K_H_beta=(b-40)*(1.315-1.324)/(40-80)+1.315#查表10-4，插值法得到(b-40)*(1.315-1.324)/(40-80)+1.315
        else :
            K_H_beta=(b-40)*(1.324-1.315)/(80-40)+1.315#查表10-4，插值法得到(b-40)*(1.324-1.315)/(80-40)+1.315
...
####按齿根弯曲疲劳强度设计####
        ...
        if gear.number==1:
            Y_Fa1=(25.177667-25)/(26-25)*(2.60-2.62)+2.62#查表10-5得到#插值法(25.177667-25)/(26-25)*(2.60-2.62)+2.62
            Y_Fa2=(116.0362-100)/(150-100)*(2.14-2.18)+2.18#查表10-5得到#插值法(116.0362-100)/(150-100)*(2.14-2.18)+2.18
            Y_Sa1=(25.177667-25)/(26-25)*(1.595-1.59)+1.59#查表10-5得到#插值法(25.177667-25)/(26-25)*(1.595-1.59)+1.59
            Y_Sa2=(116.0362-100)/(150-100)*(1.83-1.79)+1.79#查表10-5得到#插值法(116.0362-100)/(150-100)*(1.83-1.79)+1.79
            K_FN1=(3*10**6/N1)**0.02#查图10-18得
            K_FN2=(3*10**6/N2)**0.02#查图10-18得
        else :
            Y_Fa1=(21.893623-21)/(22-21)*(2.72-2.76)+2.76#查表10-5,插值法得到(21.893623-21)/(22-21)*(2.72-2.76)+2.76
            Y_Fa2=(84.290449-80)/(90-80)*(2.20-2.22)+2.22#查表10-5，插值法得到(84.290449-80)/(90-80)*(2.20-2.22)+2.22
            Y_Sa1=(21.893623-21)/(22-21)*(1.57-1.56)+1.56#查表10-5，插值法得到(21.893623-21)/(22-21)*(1.57-1.56)+1.56
            Y_Sa2=1.78#查表10-5，插值法得到(84.290449-80)/(90-80)*(1.78-1.77)+1.77
            K_FN1=(3*10**6/N1)**0.02#查图10-18得
            K_FN2=(3*10**6/N2)**0.02#查图10-18得
        ...
        print('\t         弯曲疲劳寿命系数 K_FN1={:.8},K_FN2={:.8}'.format(K_FN1,K_FN2))
        print('\t         弯曲疲劳安全系数 S=',S)
        ...
        if K_A*F_t1/b>100:
            ...
            K_F_alpha=1.4
        if gear.number==1:
            K_H_beta=(b-40)*(1.315-1.324)/(40-80)+1.315#查表10-4得，插值法得到
            K_F_beta=1.1#查图 10-13得
        else :
            K_H_beta=(b-40)*(1.324-1.315)/(80-40)+1.315##查表10-4，插值法得到
            K_F_beta=1.1#查图 10-13得
